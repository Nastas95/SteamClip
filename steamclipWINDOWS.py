...
def handle_exception(exc_type, exc_value, exc_traceback):
    ...
    try:
        ...
        windows_edition = platform.win32_edition() if hasattr(platform, 'win32_edition') else "Unknown Edition"
        windows_system = platform.system()
        windows_info = f"{windows_system} {windows_release} ({windows_edition})"
    except Exception:
...
class SteamClipApp(QWidget):
    ...
    os.environ.get('LOCALAPPDATA', os.path.expanduser("~")), 'SteamClip')
...
###        self.clear_selection_layout.addWidget(self.debug_button) #DEBUG CRASH
    ...
    def get_clip_duration(self, clip_folder):
        ...
        for session_mpd_path in session_mpd_files:
            try:
                import xml.etree.ElementTree as ET
                tree = ET.parse(session_mpd_path)
                root = tree.getroot()
