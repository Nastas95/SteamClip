...
class SteamClipApp(QWidget):
    ...
    GAME_IDS_DB = os.path.join(CONFIG_DIR, 'GameIDs.db')
    ...
    def __init__(self):
        ...
        # Create database for Game IDs
        self.create_gameid_database()
    ...
    def create_gameid_database(self):
        # Create a database and table for GameIDs
        conn = sqlite3.connect(self.GAME_IDS_DB)
        cursor = conn.cursor()
        cursor.execute('''
    ...
    def insert_or_update_gameid(self, appid, name):
        # Insert or update GameID in the database
        conn = sqlite3.connect(self.GAME_IDS_DB)
        cursor = conn.cursor()
        cursor.execute('''
    ...
    def load_game_ids_from_db(self):
        # Load Game IDs from the database
        conn = sqlite3.connect(self.GAME_IDS_DB)
        cursor = conn.cursor()
        cursor.execute('SELECT appid, name FROM games')
        game_ids = {str(row[0]): row[1] for row in cursor.fetchall()}
        conn.close()
        return game_ids
    ...
    def fetch_game_ids(self):
        """Fetch game IDs from the Steam API and save them to the database."""
        logging.info("Fetching Game IDs from Steam API...")
        command = ['curl', '-s', self.STEAM_API_URL]
        try:
            result = subprocess.run(command, capture_output=True, check=True)
            data = json.loads(result.stdout.decode('utf-8'))

            # Inizia una transazione per migliorare la scrittura
            conn = sqlite3.connect(self.GAME_IDS_DB)
            cursor = conn.cursor()

            # Cancella i dati esistenti prima di aggiornare
            ...
            logging.info("Game IDs fetched successfully.")
        except subprocess.CalledProcessError as e:
            logging.error(f"Failed to fetch game names from Steam API: {e}")
            self.show_error("Failed to fetch game names from Steam API.")

    def load_game_ids(self):
        """Load game IDs from the database."""
        if not self.is_connected():
            if not os.path.exists(self.GAME_IDS_DB):
        ...
        self.game_ids = self.load_game_ids_from_db()
