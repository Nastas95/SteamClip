...
Download SteamClip.sh from the Release page or clone the repository.
...
USAGE
    ...
    Run the script by double clicking it. Upon launch the program will check if the userdata folder containing your SteamID is in the default directory (~/.local/share/Steam/userdata) or previusly selected custom directory. If for some reason the expected userdata folder is invalid, the program will prompt you to select a valid userdata folder. Config file is located in ~/.config/SteamClip
